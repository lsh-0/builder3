...
def keypair_path(iid):
    path = project.instance_path(iid, create_dirs=True)
    pem_fname = iid + "_rsa"
    pub_fname = pem_fname + ".pub"
    # absolute paths. is this what I want? 
    # context.py shortens them, but they could be shortened right here
...
def create_keypair(iid):
    ...
    rc = os.system(cmd) # TODO: replace with utils.local_cmd
